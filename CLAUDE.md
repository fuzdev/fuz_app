...
- ./auth/ — Auth domain (crypto, schema, queries, middleware, routes, deps)
  ...
  - Routes:
    ...
    - `admin_routes.ts` — Admin session and token revoke-all routes. `create_admin_account_route_specs(deps: {log, on_audit_event})`. Account listing is **RPC-only** (`admin_account_list` in `admin_actions.ts`); permit grant/revoke are **RPC-only** (`permit_offer_create` + `permit_revoke` in `permit_offer_actions.ts`).
  ...
  - Actions (SAES):
    - `admin_actions.ts` — `create_admin_actions(deps, options?)` — admin-only RPC actions (currently `admin_account_list`: `side_effects: false`, `z.null()` input, `{accounts: AdminAccountEntryJson[], grantable_roles: RoleName[]}` output). Admin-role gate enforced in handler (mirrors `permit_revoke`). `ADMIN_ACCOUNT_LIST_METHOD` constant; `AdminAccountListInput`/`AdminAccountListOutput` schemas paired with same-named `z.infer` type exports. `AdminActionDeps = Pick<RouteFactoryDeps, 'log' | 'on_audit_event'>` (on_audit_event reserved for future admin mutations). `grantable_roles` derived from `role_options` in the factory closure.
