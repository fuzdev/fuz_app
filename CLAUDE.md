...
- **runtime/** — composable `*Deps` interfaces (`EnvDeps`, `FsReadDeps`, `FsWriteDeps` (`mkdir`/`write_text_file`/`write_file`/`rename` + `fsync` — the durability seam the fact disk CAS calls on a temp before its publishing rename; mock no-ops it), `FsRemoveDeps` (`remove`), `FsStreamDeps` (`read_file_stream`/`write_file_stream` — bounded-memory streaming for GB-scale transfer), `FetchDeps`, `CommandDeps`; bundled `RuntimeDeps`; `StatResult` carries `size` (byte length) + `mtime_ms` (epoch ms, populated by node/deno, omitted by the mock — read by the fact-store orphan-temp sweep)). Implementations: `create_node_runtime`, `create_deno_runtime`, `create_mock_runtime` (+ `MockExitError`). `write_file_atomic` in `fs.ts`.
