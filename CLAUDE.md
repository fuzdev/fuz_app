...
- **server/** — two-step assembly: `create_app_backend` (deps + DB + `close`) then `create_app_server` (requires initialized backend). `AppServerContext` carries `audit_sse` + `app_settings`. Also env validation (`validate_server_env`), multi-phase SvelteKit static fallback, `log_startup_summary`, `validate_nginx_config`.
