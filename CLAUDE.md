...
- ./ui/ — Frontend components, state, and layout primitives
  ...
  - State: `loadable.svelte.ts` (base `Loadable`), `auth_state.svelte.ts` (`AuthState`, `auth_state_context`; incl. `signup()`), `account_sessions_state.svelte.ts`, `audit_log_state.svelte.ts` (fetch + SSE streaming via `subscribe()`), `admin_accounts_state.svelte.ts` (narrow `AdminAccountsRpc` for offer retract; grant/revoke still REST), `admin_invites_state.svelte.ts`, `app_settings_state.svelte.ts`, `admin_sessions_state.svelte.ts`, `table_state.svelte.ts`, `form_state.svelte.ts` — `FormState` (Enter-advance, blur-touched via `focusout`, `show(field)`/`focus(field)`)
