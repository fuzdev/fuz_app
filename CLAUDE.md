...
- ./actions/ — SAES action spec system + runtime
  ...
  - `cancel.ts` — `cancel_action` — client→server cancel notification (`CancelNotificationParams`)
...
- ./ui/ — Frontend components, state, and layout primitives
  ...
  - State: `loadable.svelte.ts` (base `Loadable`), `auth_state.svelte.ts` (`AuthState`, `auth_state_context`; incl. `signup()`), `account_sessions_state.svelte.ts`, `audit_log_state.svelte.ts` (narrow `AuditLogRpc` for `list` + `permit_history`; `fetch` / `fetch_permit_history` / SSE gap-fill flow through the adapter; `has_rpc` getter; SSE stream continues via `EventSource` directly; exports `audit_log_rpc_context`), `admin_accounts_state.svelte.ts` (narrow `AdminAccountsRpc` for `list_accounts`/grant/revoke/retract — every operation flows through the RPC adapter; `has_rpc` gates the entire surface so `fetch()` also requires the adapter; `revoke_permit({actor_id, permit_id, reason?})`; exports `admin_accounts_rpc_context`), `admin_invites_state.svelte.ts` (narrow `AdminInvitesRpc` for `list`/`create`/`delete`; `has_rpc` gates all operations; mirrors `AdminAccountsRpc` / `AuditLogRpc`; exports `admin_invites_rpc_context`), `app_settings_state.svelte.ts` (narrow `AppSettingsRpc` for `get`/`update`; `has_rpc` gates `fetch`/`update_open_signup`; mirrors the other admin Rpc adapters — no REST fallback; exports `app_settings_rpc_context`), `admin_sessions_state.svelte.ts` (consumes `admin_accounts_rpc_context` for the two revoke-all mutations), `table_state.svelte.ts`, `form_state.svelte.ts` — `FormState` (Enter-advance, blur-touched via `focusout`, `show(field)`/`focus(field)`). All four `*_rpc_context` helpers have a `() => null` fallback so components render the "rpc adapter not wired" message when mounted outside a provisioner; consumers call `context.set(() => rpc)` once at the admin route shell and every `Admin*.svelte` / `OpenSignupToggle.svelte` consumes via context — RPC adapters are never threaded through props.
