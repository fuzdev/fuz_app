...
- ./auth/ — Auth domain (crypto, schema, queries, middleware, routes, deps)
  ...
  - Routes:
    ...
    - `admin_routes.ts` — Admin routes: list accounts, revoke sessions, revoke tokens. `create_admin_account_route_specs(deps: {log, on_audit_event}, options?)`. Permit grant/revoke are **RPC-only** — see `permit_offer_create` + `permit_revoke` in `permit_offer_actions.ts`.
...
- ./ui/ — Frontend components, state, and layout primitives
  ...
  - State: `loadable.svelte.ts` (base `Loadable`), `auth_state.svelte.ts` (`AuthState`, `auth_state_context`; incl. `signup()`), `account_sessions_state.svelte.ts`, `audit_log_state.svelte.ts` (fetch + SSE streaming via `subscribe()`), `admin_accounts_state.svelte.ts` (narrow `AdminAccountsRpc` for grant/revoke/retract — all three drive the RPC surface; `has_rpc` boolean replaces the previous `can_retract`; `revoke_permit({actor_id, permit_id, reason?})`), `admin_invites_state.svelte.ts`, `app_settings_state.svelte.ts`, `admin_sessions_state.svelte.ts`, `table_state.svelte.ts`, `form_state.svelte.ts` — `FormState` (Enter-advance, blur-touched via `focusout`, `show(field)`/`focus(field)`)
