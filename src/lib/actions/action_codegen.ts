...
export const generate_phase_handlers = (
	...
	return `${to_action_property_key(method)}?: {\n\t\t${phase_handlers};\n\t}`;
};
...
/**
 * Derive the exported spec identifier for a method. Method names that aren't
 * legal JS identifiers — e.g. the `/` in the `peer/ping` protocol action — have
 * each run of illegal characters collapsed to a single `_`, so `peer/ping`
 * resolves to `peer_ping_action_spec` (the export in `actions/peer_ping.ts`).
 * Already-valid names pass through unchanged.
 */
export const to_action_spec_identifier = (method: string): string =>
	`${method.replace(/[^A-Za-z0-9_$]+/g, '_')}_action_spec`;
...
/**
 * Render a method name as an object/interface property key — bare when it's a
 * valid JS identifier, single-quoted otherwise. Method names like `peer/ping`
 * aren't legal as bare keys, so quoting keeps the emitted `ActionSpecs` /
 * `ActionInputs` / `FrontendActionsApi` members valid TypeScript. Pairs with
 * the bracketed indexed-access form (`ActionInputs['peer/ping']`) used for
 * value/type lookups, which already accepts the raw method string.
 */
export const to_action_property_key = (method: string): string =>
	/^[A-Za-z_$][A-Za-z0-9_$]*$/.test(method) ? method : `'${method}'`;
...
export const generate_actions_api_method_signature = (
	...
	return `${to_action_property_key(spec.method)}: (${input_param}${options_param}) => ${return_type};`;
};
...
export const resolve_spec_qualifier = (
	...
	return (s) => `specs.${to_action_spec_identifier(s.method)}`;
};
...
export const action_specs: Array<ActionSpecUnion> = Object.values(ActionSpecs);`;
	...
	const value_lines = filtered
		.map((s) => `\t${to_action_property_key(s.method)}: ${qualify(s)},`)
		.join('\n');
	const type_lines = filtered
		.map((s) => `\t${to_action_property_key(s.method)}: typeof ${qualify(s)};`)
		.join('\n');
...
export interface ActionOutputs {}`;
	...
	const inputs_value = filtered
		.map((s) => `\t${to_action_property_key(s.method)}: ${qualify(s)}.input,`)
		.join('\n');
	const inputs_type = filtered
		.map((s) => `\t${to_action_property_key(s.method)}: z.infer<typeof ${qualify(s)}.input>;`)
		.join('\n');
	const outputs_value = filtered
		.map((s) => `\t${to_action_property_key(s.method)}: ${qualify(s)}.output,`)
		.join('\n');
	const outputs_type = filtered
		.map((s) => `\t${to_action_property_key(s.method)}: z.infer<typeof ${qualify(s)}.output>;`)
		.join('\n');
...
export interface ActionEventDatas {}`;
	...
	const lines = filtered.map((spec) => {
		...
		return `\t${to_action_property_key(spec.method)}: ${data_type}${type_args};`;
	});
