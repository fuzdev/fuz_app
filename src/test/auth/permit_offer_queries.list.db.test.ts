/**
 * Tests for `permit_offer_queries.ts` — list / find_pending / sweep readers.
 *
 * Covers the active-offer filter on `query_permit_offer_list`, the
 ...
 * lives in `permit_offer_queries.supersede.db.test.ts`.
 *
 * @module
 */
...
describe_db('permit_offer_queries.list', (get_db) => {
	test('list filters out every terminal state plus expired-pending', async () => {
		...
		await query_permit_offer_retract(deps, retractable.id, grantor.actor_id);
	...
	test('find_pending returns null for every terminal state plus expired-pending', async () => {
		...
		assert.strictEqual(await query_permit_offer_find_pending(deps, acceptable.id), null);
		...
		await query_permit_offer_decline(deps, declinable.id, recipient.account_id, null);
		assert.strictEqual(await query_permit_offer_find_pending(deps, declinable.id), null);
		...
		await query_permit_offer_retract(deps, retractable.id, grantor.actor_id);
		assert.strictEqual(await query_permit_offer_find_pending(deps, retractable.id), null);

		// superseded
		const superseded_id = await insert_superseded_offer(db, grantor, recipient, {
			scope_id: create_uuid(),
		});
		assert.strictEqual(await query_permit_offer_find_pending(deps, superseded_id), null);
		...
		assert.strictEqual(await query_permit_offer_find_pending(deps, expired.id), null);

		// missing
		assert.strictEqual(await query_permit_offer_find_pending(deps, create_uuid()), null);
	});
	...
	test('list orders by expires_at ASC (soonest first)', async () => {
		// `query_permit_offer_list` ORDER BY expires_at ASC is part of the
