/**
 * Tests for `permit_offer_queries.ts` — offer creation paths.
 *
...
import {make_account, future, hour} from './permit_offer_queries.fixtures.js';

describe_db('permit_offer_queries.create', (get_db) => {
	...
	test('re-offer narrows account-grain row to a specific actor (to_actor_id null → set)', async () => {
		...
		const account_grain = await query_permit_offer_create(deps, {
			from_actor_id: grantor.actor_id,
			to_account_id: recipient.account_id,
			role: 'teacher',
			expires_at: future(hour),
		...
		const narrowed = await query_permit_offer_create(deps, {
			from_actor_id: grantor.actor_id,
			to_account_id: recipient.account_id,
	...
	test('re-offer widens actor-grain row back to account-grain (to_actor_id set → null)', async () => {
		...
		const actor_grain = await query_permit_offer_create(deps, {
			from_actor_id: grantor.actor_id,
			to_account_id: recipient.account_id,
			to_actor_id: recipient.actor_id,
			role: 'teacher',
		...
		const widened = await query_permit_offer_create(deps, {
			from_actor_id: grantor.actor_id,
			to_account_id: recipient.account_id,
			to_actor_id: null,
			role: 'teacher',
