...
import {query_grant_permit, query_revoke_permit} from '$lib/auth/permit_queries.js';
 ...
 * (`SQLSTATE 40P01`). The `revoke_permit` × `accept_offer` race deadlocks
 * deterministically when ordering interleaves: accept holds the offer
 * row-lock and waits on the active-permit partial-unique-index conflict
 * with revoke; revoke holds the permit row-lock and its supersede CTE
...
describe_pg('permit_offer_queries.concurrent', (get_db) => {
	...
	test('concurrent permit_revoke + offer_accept never leaves an active permit at the tuple', async () => {
		// Setup: permit P active at (recipient.actor, 'teacher', null); pending
		// sibling offer O at the same tuple from the same grantor (the partial
		// pending-unique index keys on `from_actor_id` so an active permit
		...
		const [revoke_result, accept_result] = await Promise.all([
			run_with_deadlock_retry(db, (tx) =>
				query_revoke_permit({db: tx}, permit.id, recipient.actor_id, grantor.actor_id, 'race-test'),
			),
			run_with_deadlock_retry(db, (tx) =>
				query_accept_offer(
					{db: tx},
		...
		// active permit at the tuple. `query_revoke_permit`'s sibling-supersede
		...
		// only permit it can resolve at the tuple is the one being revoked).
		...
		assert.strictEqual(
			...
			0,
			'no active permit should remain at (recipient.actor, teacher, null)',
		);
		...
		// Accept either succeeds (offer points to the now-revoked permit) or
		// surfaces `PermitOfferAlreadyTerminalError` (revoke's supersede won).
		if (accept_result.status === 'rejected') {
			...
			// or already-revoked) permit P, not a fresh permit at the tuple.
			assert.strictEqual(
				accept_result.value.permit.id,
				permit.id,
				'accept must resolve to the existing permit P (no fresh permit can land at the tuple)',
			);
			// Offer is accepted, resulting_permit_id points to P.
			const row = await db.query_one<{
				accepted_at: string | null;
				resulting_permit_id: string | null;
			}>(`SELECT accepted_at, resulting_permit_id FROM permit_offer WHERE id = $1`, [offer.id]);
			assert.ok(row, 'offer row should exist');
			assert.ok(row.accepted_at, 'accepted_at should be stamped');
			assert.strictEqual(row.resulting_permit_id, permit.id);
		}
	});
});
