...
describe_db('RoleEscalation', (get_db) => {
	test('admin with only admin role cannot directly grant keeper at query level', async () => {
		...
		assert.strictEqual(role_grant.role, ROLE_KEEPER);
		assert.strictEqual(role_grant.granted_by, admin_id);
	});
	...
	test('actor with admin role cannot see other actor role_grants via find_active_for_actor', async () => {
		...
		assert.strictEqual(admin_role_grants.length, 1);
		assert.strictEqual(admin_role_grants[0]!.role, ROLE_ADMIN);

		// querying with other's actor_id returns other's role_grants
		const other_role_grants = await query_role_grant_find_active_for_actor(deps, other_id);
		assert.strictEqual(other_role_grants.length, 1);
		assert.strictEqual(other_role_grants[0]!.role, ROLE_KEEPER);
	});
});
