...
import {permit_offer_list_action_spec} from '$lib/auth/permit_offer_action_specs.js';
...
describe_db('permit_offer.multi_actor — dispatcher_400', (get_db) => {
	...
	describe('dispatcher-level multi-actor 400', () => {
		test('authenticated request with multi-actor account hits 400 actor_required envelope before the handler runs', async () => {
			...
			// `permit_offer_list` is `side_effects: false` so it exercises
