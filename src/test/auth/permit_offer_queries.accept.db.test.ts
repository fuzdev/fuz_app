/**
 * Tests for `permit_offer_queries.ts` — `query_accept_offer` core paths.
 *
 * Covers the happy-path accept (permit insert + audit fan-out + has_role),
 ...
 * live in `permit_offer_queries.supersede.db.test.ts`.
 *
 * @module
 */
...
describe_db('permit_offer_queries.accept', (get_db) => {
	test('accept inserts permit + stamps resulting_permit_id + emits audit events', async () => {
		...
		// the permit grant references the resulting permit. Pin index, not
		// just multiset.
		const [accept_event, grant_event] = result.audit_events;
		assert.ok(accept_event);
		assert.ok(grant_event);
		assert.strictEqual(accept_event.event_type, 'permit_offer_accept');
		assert.strictEqual(grant_event.event_type, 'permit_grant');
		...
		assert.strictEqual(grant_metadata.permit_id, result.permit.id);
	...
	test('accept rejects when actor_id does not belong to to_account_id (defense-in-depth)', async () => {
		...
		assert.strictEqual(still_pending.resulting_permit_id, null);
	});
});
