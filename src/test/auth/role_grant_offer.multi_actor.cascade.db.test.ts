 ...
 *
 * Audit envelopes for `role_grant_offer_retract`, `role_grant_offer_decline`,
 * `role_grant_offer_expire`, and the in-tx `role_grant_offer_supersede` event
...
describe_db('role_grant_offer.multi_actor — cascade', (get_db) => {
	...
	describe('cascade inheritance', () => {
		test('actor-targeted retract carries the actor on the audit envelope', async () => {
			...
			const retract_event = events.find((e) => e.event_type === 'role_grant_offer_retract');
		...
		test('actor-targeted decline still puts the grantor in both target columns', async () => {
			...
			const decline_event = events.find((e) => e.event_type === 'role_grant_offer_decline');
		...
		test('expired actor-targeted offer carries the actor on the role_grant_offer_expire envelope', async () => {
