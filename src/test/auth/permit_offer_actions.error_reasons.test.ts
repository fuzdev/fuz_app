...
import * as permit_offer_specs from '$lib/auth/permit_offer_action_specs.js';
import * as error_schemas from '$lib/http/error_schemas.js';

const handler_source = readFileSync(
	resolve(dirname(fileURLToPath(import.meta.url)), '../../lib/auth/permit_offer_actions.ts'),
	'utf8',
);

const error_value_by_name: Record<string, string> = {};
for (const [name, value] of Object.entries({...permit_offer_specs, ...error_schemas})) {
...
const all_specs = permit_offer_specs as Record<string, unknown>;
...
for (const [spec_const, handler_name] of spec_to_handler) {
	...
	test(`${spec?.method ?? spec_const}: declared error_reasons match handler throws`, () => {
		assert(spec, `spec const ${spec_const} not exported from permit_offer_action_specs.js`);
