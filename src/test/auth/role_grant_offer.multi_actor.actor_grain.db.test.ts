 ...
 * reject with `role_grant_offer_actor_mismatch`. Offers targeted at an
 * actor that doesn't belong to `to_account_id` reject up-front with
 * `role_grant_offer_actor_account_mismatch`. Self-target check still fires when
...
describe_db('role_grant_offer.multi_actor — actor_grain', (get_db) => {
	...
	describe('actor-grain offers (`to_actor_id` set)', () => {
		...
		test('action-level wrong-actor accept maps RoleGrantOfferActorMismatchError to ERROR_ROLE_GRANT_OFFER_ACTOR_MISMATCH', async () => {
