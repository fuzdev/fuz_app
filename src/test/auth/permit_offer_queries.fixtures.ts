/**
 * Shared scaffolding for the `permit_offer_queries.<aspect>.db.test.ts`
 * sibling suites.
 *
 ...
 * Mirrors the `permit_offer.multi_actor.fixtures.ts` pattern on the
 * actions side.
 *
...
import type {PermitOffer} from '$lib/auth/permit_offer_schema.js';
...
/**
 * Test helper — raw INSERT a superseded `permit_offer` row.
 *
 * No public API sets `superseded_at` directly (callers go through accept or
 * permit revoke). Tests for the list/find_pending/sweep predicates need
