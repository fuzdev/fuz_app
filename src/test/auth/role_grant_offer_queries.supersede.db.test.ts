/**
 * Tests for `role_grant_offer_queries.ts` — multi-grantor coexistence + supersede.
 *
...
describe_db('role_grant_offer_queries.supersede', (get_db) => {
	...
	test('accept supersedes sibling pending offers and emits audit events', async () => {
		...
		// Locks in single-terminal invariant (role_grant_offer_single_terminal CHECK).
		...
		// audit events: role_grant_offer_accept → role_grant_create → 2× role_grant_offer_supersede
		// Pin order — accept fires first (offer side), grant second (role_grant
		...
		assert.strictEqual(result.audit_events[0]?.event_type, 'role_grant_offer_accept');
	...
	test('history_for_account returns offers in both directions, newest first, with pagination', async () => {
		...
		// `query_role_grant_offer_history_for_account` is documented as
		...
		const for_grantor = await query_role_grant_offer_history_for_account(deps, grantor.account_id);
		...
		const page1 = await query_role_grant_offer_history_for_account(deps, grantor.account_id, 1, 0);
		assert.strictEqual(page1.length, 1);
		assert.strictEqual(page1[0]!.id, incoming.id);
		const page2 = await query_role_grant_offer_history_for_account(deps, grantor.account_id, 1, 1);
	...
	test('decline on A does not affect B from a different grantor', async () => {
		...
		// B is still pending.
		const still = await query_role_grant_offer_find_pending(deps, offer_b.id);
		assert.ok(still);
	...
	test('revoke-bypass regression: accept A → revoke → cannot accept superseded B', async () => {
		...
		// Admin revokes the resulting role_grant.
		const revoke = await query_revoke_role_grant(
			deps,
		...
		// Emit the role_grant_revoke audit event like the route layer does, so the
		...
		assert.strictEqual(by_type.get('role_grant_offer_accept')?.length, 1);
		...
		assert.strictEqual(by_type.get('role_grant_revoke')?.length, 1);
	});
});
