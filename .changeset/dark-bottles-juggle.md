...
- `apply_route_specs` and `create_rpc_endpoint` gain an authorization phase before input validation. Per-route order: params → query → pre-validation auth (401) → authorization → post-authorization auth (403) → input validation → handler. `acting` is read from raw query (GET) or pre-parsed body (mutations); Hono caches the parse so input validation does not re-parse.
...
- `ERROR_ACTOR_REQUIRED`, `ERROR_ACTOR_NOT_ON_ACCOUNT`, `ERROR_NO_ACTORS_ON_ACCOUNT` in `error_schemas.ts`.
